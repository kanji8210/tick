...
const InsuredDashboard = ({ user, onNavigate, initialTab }) => {
  ...
  /** Open a protected sale document in a new tab. */
  const handleViewDocument = async (saleId, documentType) => {
    const loadingKey = `${saleId}-${documentType}`;
    setActionLoading(loadingKey);
    const numId = toNumericId(saleId);
    const url = documentType === 'certificate'
      ? `${WP_REST_BASE}/maljani/v1/certificate/${numId}`
      : documentType === 'policy-document'
        ? `${WP_REST_BASE}/maljani/v1/policy-document/${numId}`
      : `${WP_REST_BASE}/maljani/v1/invoice/${numId}?doc_type=${documentType}`;
    const hdrs = restHeaders();
    try {
      const res = await fetch(url, { headers: hdrs });
      if (!res.ok) {
        throw new Error(await readRestError(res, `Could not load ${documentType}`));
      }
      ...
      if (data.html) {
        ...
        throw new Error(`No ${documentType} content returned`);
      }
    } catch (e) {
      alert(e.message);
    } finally {
                      ...
                      <div style={{ flex: 1, minWidth: 200 }}>
                        ...
                        {isPending && (
                          <div style={{ marginTop: 18, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            <button
                              onClick={() => handleViewDocument(p.id, 'invoice')}
                              disabled={actionLoading === `${p.id}-invoice`}
                              title="Payment invoice — generated by the insurer"
                            ...
                            >
                              {actionLoading === `${p.id}-invoice` ? 'Loading…' : '📄 Invoice'}
                            </button>
                        ...
                        {!isPending && (
                          <div style={{ marginTop: 12, display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            {isConfirmed && <>
                              <span style={{ color: 'var(--glass-border)', fontSize: 11 }}>|</span>
                              <button
                                onClick={() => handleViewDocument(p.id, 'receipt')}
                                disabled={actionLoading === `${p.id}-receipt`}
                                style={{ background: 'none', border: 'none', color: '#86efac', fontSize: 12, cursor: 'pointer', textDecoration: 'underline', fontFamily: 'var(--font-body)', padding: 0 }}
                              >
                                {actionLoading === `${p.id}-receipt` ? 'Loading…' : 'Receipt'}
                              </button>
                              <span style={{ color: 'var(--glass-border)', fontSize: 11 }}>|</span>
                              <button
                                onClick={() => handleViewDocument(p.id, 'certificate')}
                                disabled={actionLoading === `${p.id}-certificate`}
                                style={{ background: 'none', border: 'none', color: '#86efac', fontSize: 12, cursor: 'pointer', textDecoration: 'underline', fontFamily: 'var(--font-body)', padding: 0 }}
                              >
                                {actionLoading === `${p.id}-certificate` ? 'Loading…' : 'Certificate'}
                              </button>
                            </>}
                            {isConfirmed && ['active', 'verification_ready'].includes(p.policyStatus) && <>
                              <span style={{ color: 'var(--glass-border)', fontSize: 11 }}>|</span>
                              <button
                                onClick={() => handleViewDocument(p.id, 'policy-document')}
                                disabled={actionLoading === `${p.id}-policy-document`}
                                style={{ background: 'none', border: 'none', color: '#86efac', fontSize: 12, cursor: 'pointer', textDecoration: 'underline', fontFamily: 'var(--font-body)', padding: 0 }}
                              >
                                {actionLoading === `${p.id}-policy-document` ? 'Loading…' : 'Insurance document'}
                              </button>
                            </>}
                            {isConfirmed && !['active', 'verification_ready'].includes(p.policyStatus) && (
                              <span title="The insurer document will appear after manual issuance." style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12 }}>
                                Insurer document pending
                              </span>
                            )}
                          </div>
